...
(() => {
  ...
  function setVersionLabels(){
    document.title = 'Мой ватчлист · v5.6.1';
    const eyebrow = document.querySelector('.eyebrow');
    if (eyebrow) eyebrow.textContent = eyebrow.textContent.replace(/v\d+(?:\.\d+)?/i, 'v5.6.1');
    const footer = document.querySelector('footer');
    if (footer) {
      for (const node of [...footer.childNodes]) {
        if (node.nodeType === Node.TEXT_NODE && /Версия\s+v/i.test(node.textContent || '')) {
          node.textContent = (node.textContent || '').replace(/Версия\s+v\d+(?:\.\d+)?/i, 'Версия v5.6.1');
        }
      }
      footer.innerHTML = footer.innerHTML.replace(/Версия\s+v\d+(?:\.\d+)?/i, 'Версия v5.6.1');
    }
  }
  ...
  function downloadPortableBackup(filename='мой-ватчлист-v5.6.1.json'){
    ...
    toast('Резервная копия v5.6.1 сохранена');
  }
  ...
  openExport = function(){
    ...
    openModal(`<h3>📤 Импорт, экспорт и перенос</h3>
      <p class="hint">Полная копия v5.6.1.1 содержит прогресс, добавленные фильмы/сериалы, TMDB-постеры и рейтинг, настройки и TMDB Token.</p>
      <div class="result-card">
  ...
  function updateStorageBanner(){
    ...
    b.innerHTML = `<span>📲 <b>Установленная PWA-копия</b> · добавлено: <b>${added}</b> · TMDB-кэш: <b>${meta}</b> · токен: <b>${token?'✓':'нет'}</b>${(!token && added===0 && meta===0)?' · Похоже, данные из Safari ещё не перенесены.':''}</span>
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ${token?'':`<button class="btn" id="v561PasteToken">🔑 Вставить TMDB Token</button>`}
    ...
    const paste = b.querySelector('#v561PasteToken');
    if (paste) paste.onclick = () => {
      const key = prompt('Вставь TMDB API Read Access Token:');
      if (!key || !key.trim()) return;
      tmdbSetKey(key.trim());
      saveState();
      updateStorageBanner();
      updateTmdbStatusV56('токен сохранён');
      toast('TMDB Token сохранён');
      setTimeout(() => refreshExistingFromTmdb({silent:true}), 300);
    };
  }
    ...
    updateBtn.onclick = async () => {
      ...
      try {
        ...
        if ('caches' in window) {
          ...
          await Promise.all(keys.filter(k => /^watchlist-v/i.test(k) && k !== 'watchlist-v5-6-1').map(k => caches.delete(k)));
        }
      } catch(e) {
        console.warn('Update cleanup', e);
      }
      const u = new URL(location.href);
      u.searchParams.set('appv', '561-' + Date.now());
  ...
  updateStorageBanner();
  updateTmdbStatusV56();
  const activeStatus = document.getElementById('tmdbStatus');
  if (activeStatus && !document.getElementById('v561Active')) {
    const badge = document.createElement('span');
    badge.id = 'v561Active';
    badge.style.cssText = 'display:inline-block;margin-left:8px;padding:2px 7px;border:1px solid var(--green);border-radius:999px;color:var(--green);font-weight:700';
    badge.textContent = 'v5.6.1 ACTIVE';
    activeStatus.appendChild(badge);
  }
  render();
  ...
  console.info('Watchlist v5.6.1 patch active', {
